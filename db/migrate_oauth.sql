...
-- 2. Wipe disposable user data (cascades to anything referencing it).
...
-- 3. Drop the FKs to auth.users, then widen user_id to text (Google sub).
...
-- 4. Drop the old RLS self-read policies. They used auth.uid() (a Supabase
--    session function) which never matches our own auth, so all access is via
--    the service-role backend. Dropping avoids a confusing half-applied state.
